...
This process uses the `PKGBUILD` to create and install a package that can be managed by `pacman`.
...
> **Note:** Using `ansi-*` colors as a `source` for file-based themes is a planned feature and is not yet implemented.
> This example shows the future intended usage.
